...
export default class Kategori extends Component {
	...
	componentDidMount(){
		axios({
			method: 'GET',
			url: 'https://pixabay.com/api/?key=7528484-47ecdc7e30a3217e3ce3d8530&category='+this.props.navigation.getParam('kategori')+'&lang=id&image_type=all&pretty=true'
		}).then((response) => {
	...
	render() {
        ...
        <Header>
      <Body>
      <Text style={{color:'white'}}>Kategori : {this.props.navigation.getParam('kategori')}</Text>
      </Body>
        </Header>
        <Content padder>
